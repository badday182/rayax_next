...
export const KolinnyiSuhlob = () => {
  const [selectedKolinnyiSuhlobViews, setSelectedKolinnyiSuhlobViews] =
    useState(kolinnyiSuhlobViews[0]);
  const [kolinnyiSuhlobCounter, setKolinnyiSuhlobCounter] = useState([
    { id: uuidv4() },
  ]);
  ...
  return (
    <div className="">
      <AddOptionBlock
        ...
        onAddClick={() =>
          handleAddOption(setKolinnyiSuhlobCounter, kolinnyiSuhlobCounter)
        }
        onDeleteClick={(deleteId) =>
          handleDeleteOption(deleteId, [
            kolinnyiSuhlobCounter,
            setKolinnyiSuhlobCounter,
          ])
        }
      />
    </div>
